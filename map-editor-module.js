...
(function () {
  ...
  let currentUserEmail = null;
    ...
    function toggleAddMode() {
      ...
      if (addMode && typeof showToast === 'function') {
        showToast('Tap the map to place the pin', 'success');
      }
    }
  ...
  // add-pin button shows. Desktop keeps the button even while the panel
  // is open (they don't overlap); on mobile the bottom sheet covers its
  // corner, so it hides until the sheet closes.
  function editorPanelVisibility() {
    ...
    const fab = document.getElementById('editor-mobile-fab');
    if (fab) fab.style.display = (userOn && (!mobile || !infoOpen)) ? 'flex' : 'none';
  }
  ...
  function injectEditorDom() {
    ...
    // Floating pin button (all breakpoints): THE add-pin control. A
    // click arms add-mode (toggleAddMode turns it green and the next map
    // click places the pin, opening the panel on the new pin's form);
    // clicking again cancels.
    const fab = document.createElement('button');
    fab.id = 'editor-mobile-fab';
    fab.type = 'button';
    fab.title = 'Add a map pin';
    fab.setAttribute('aria-label', 'Add a map pin');
