...
(function () {
  ...
  // --- Editor pager-window slider (Map Controls) --------------------------
  // Mirrors the main pager filter slider so editors get an obvious 1-48h
  // control right in the panel; both sliders stay in sync and drive the
  // shared pagerHours global + data reload.
  function editorPagerHoursInput() {
    const r = document.getElementById('editor-pager-hours');
    const label = document.getElementById('editor-pager-hours-value');
    if (r && label) label.textContent = r.value + 'h';
  }

  function editorPagerHoursCommit() {
    const r = document.getElementById('editor-pager-hours');
    if (!r) return;
    const main = document.getElementById('pager-hours-range');
    if (main) {
      main.value = r.value;
      // Main-panel plumbing: updates pagerHours, saves, refreshes data + fill.
      if (typeof updatePagerHoursLabel === 'function') updatePagerHoursLabel();
      if (typeof updateSliderFill === 'function') updateSliderFill();
      if (typeof onPagerHoursChange === 'function') { onPagerHoursChange(); return; }
    }
    if (typeof loadAllData === 'function') loadAllData();
  }

  function syncEditorPagerSlider() {
    const r = document.getElementById('editor-pager-hours');
    const main = document.getElementById('pager-hours-range');
    if (r && main) {
      r.value = main.value;
      editorPagerHoursInput();
    }
  }
  ...
  async function activateEditor() {
    try {
      ...
      try {
        ...
        // Keep the Map Controls slider mirroring the main one.
        syncEditorPagerSlider();
        if (range) range.addEventListener('input', syncEditorPagerSlider, { passive: true });
      } catch (e) { /* non-fatal */ }
  ...
  window.editorPagerHoursInput = editorPagerHoursInput;
  window.editorPagerHoursCommit = editorPagerHoursCommit;
})();
