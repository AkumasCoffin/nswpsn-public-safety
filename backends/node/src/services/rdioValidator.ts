...
// ---------------------------------------------------------------------------
// Trimmed polish for the rebuilder path (RDIO_REBUILDER_ENABLED). Runs
// AFTER rebuildIncidents() has reattached server-authoritative
// transcripts + units, so the surface area shrinks dramatically:
//
//   1. Drop legacy `timeline[]` if it sneaks in.
//   2. Drop incidents whose title+summary echoes a worked example from
//      prompts/rdio_hourly.txt (model-cache regurgitation).
//   3. Dedup by `member_call_ids` overlap (≥50% of smaller, or proper
//      subset). Richer incident wins.
//
// Compared to validateStructuredAgainstTranscripts: no call_id touching
// (the rebuilder already filtered to inputMap), no NSWPF redaction
// (per user spec), no NATO/unit drops (units are server-derived from
// UIDs), no agencies allow-list filter, no agency_stats.NSWPF strip.
// ---------------------------------------------------------------------------
 ...
 *  now server-attached so they count even though the LLM didn't emit
 *  them. */
