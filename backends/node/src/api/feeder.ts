...
function pagerLinuxInstaller(enrolCode: string): string {
  ...
  return [
    ...
    `udevadm control --reload-rules 2>/dev/null || true; udevadm trigger 2>/dev/null || true`,
    ``,
    `# The DVB-T kernel driver binds RTL dongles on sight and does not let go,`,
    `# which is what 'usb_claim_interface error -6' means. Blacklisting alone`,
    `# only helps the NEXT boot, so the running module is evicted here too.`,
    `cat > /etc/modprobe.d/blacklist-nswpsn-rtl.conf <<'BL'`,
    `blacklist dvb_usb_rtl28xxu`,
    `blacklist rtl2832`,
    `blacklist rtl2830`,
    `BL`,
    `# Leaf first: rtl2832 is what dvb_usb_rtl28xxu sits on, and removing the`,
    `# base while the leaf is loaded fails.`,
    `for m in dvb_usb_rtl28xxu rtl2832_sdr rtl2832 rtl2830; do`,
    `  modprobe -r "$m" 2>/dev/null || true`,
    `done`,
    `# modprobe -r says nothing when it fails, and the install would otherwise`,
    `# report success on a machine whose dongle can never be opened. Prove the`,
    `# SDR is reachable instead of assuming it.`,
    `SDR_OK=0`,
    `if command -v rtl_test >/dev/null 2>&1; then`,
    `  if timeout 10 rtl_test -t >/dev/null 2>&1; then SDR_OK=1; fi`,
    `fi`,
    `if [ "$SDR_OK" -ne 1 ]; then`,
    `  echo "" >&2`,
    `  echo "WARNING: the SDR could not be opened." >&2`,
    `  if lsmod 2>/dev/null | grep -q '^dvb_usb_rtl28xxu'; then`,
    `    echo "  The DVB-T driver is still loaded and holding it. It is now" >&2`,
    `    echo "  blacklisted, so a REBOOT will release the dongle:  sudo reboot" >&2`,
    `  else`,
    `    echo "  Nothing is claiming it via DVB, so check the dongle is plugged" >&2`,
    `    echo "  in and not in use by another program (rtl_test -t to retry)." >&2`,
    `  fi`,
    `  echo "  The node will install and start, but it will decode nothing until this is fixed." >&2`,
    `  echo "" >&2`,
    `fi`,
    ``,
    `cat > /etc/systemd/system/nswpsn-node.service <<UNIT`,
...
function adsbLinuxInstaller(enrolCode: string): string {
  ...
  return [
    ...
    `    echo "  in and not in use by another program (rtl_test -t to retry)." >&2`,
    `  fi`,
    `  echo "  The node will install and start, but dump1090 will crash-loop until this is fixed." >&2`,
    `  echo "" >&2`,
    `fi`,
    ``,
    `cat > /etc/systemd/system/nswpsn-node.service <<UNIT`,
