...
type Config struct {
	...
	// the backend in hello; defaults to adsb (the only type this agent serves).
	Kind    string `yaml:"kind"`
	DataDir string `yaml:"data_dir"`
	// JSONDir is where the decoder writes aircraft.json / stats.json and the
	// agent reads them. Defaults to a systemd RuntimeDirectory on tmpfs: the
	// decoder rewrites these files once a second, which would wear out an SD
	// card on a Pi within months if it landed on real storage.
	JSONDir string `yaml:"json_dir"`
	// Dump1090Bin is the decoder executable, resolved via PATH when bare.
	Dump1090Bin string `yaml:"dump1090_bin"`
}

const (
	defaultDataDir     = "./data"
	defaultJSONDir     = "/run/nswpsn-adsb"
	defaultDump1090Bin = "dump1090-fa"
)
...
// Require https except for a loopback dev server.
...
// (The pager agent also validates a loopback relay_addr here. This agent has
// no local listener to protect: the decoder writes JSON files and the agent
// reads them, so it opens no ports at all.)

// isLoopbackHost reports whether h is localhost or a loopback IP. Still needed
// above: a dev server on http://127.0.0.1 is exempt from the https rule.
...
func (c *Config) applyDefaults() bool {
	...
	if strings.TrimSpace(c.JSONDir) == "" {
		c.JSONDir = defaultJSONDir
		changed = true
	}
	if strings.TrimSpace(c.Dump1090Bin) == "" {
		c.Dump1090Bin = defaultDump1090Bin
		changed = true
	}
...
// ensureDirs creates data_dir plus its queue/, logs/ and decoder/ subdirs.
// json_dir is NOT created here: it is a systemd RuntimeDirectory owned by the
// service manager, and the decoder creates it if it is somewhere else.
func (c *Config) ensureDirs() error {
	for _, d := range []string{c.DataDir, c.QueueDir(), c.LogsDir(), c.DecoderDir()} {
...
// DecoderDir holds the generated decoder launch script. Its path is unique to
// this agent's data dir, which is what makes it safe for KillStale to match on
// — matching a bare "dump1090" would kill an unrelated decoder the operator
// happens to be running.
func (c *Config) DecoderDir() string { return filepath.Join(c.DataDir, "decoder") }
