...
// messages, applies pushed decoder config, sends WS pings every 30s (Cloudflare
...
import (
	...
	"github.com/AkumasCoffin/nswpsn-node/aircraft-node/internal/protocol"
...
// AdsbConfig is the applied configuration handed to the ConfigApplier.
type AdsbConfig struct {
	ConfigVersion  string
	CaptureEnabled bool
	FeedEnabled    bool
	// Gain: "" = decoder default; "auto" = the agent's adaptive gain loop
	// (NOT the dongle's hardware AGC — see internal/autogain); a number = that
	// fixed gain in dB.
	Gain string
	// Ppm, when non-nil, is the crystal correction passed to the decoder.
	Ppm *int
	// Lat/Lon are the exact antenna position from the node's own record. The
	// decoder needs them to report range at all, which is why the backend
	// makes the map pin mandatory for this node kind.
	Lat *float64
	Lon *float64
}

// ConfigApplier applies a pushed ADS-B config to the decoder supervisor and
// services component-restart commands. Implemented by the main package's
// aircraft manager.
type ConfigApplier interface {
	// Apply (re)computes and (re)starts the decoder for cfg.
	// captureEnabled=false stops it without disconnecting the agent.
	Apply(cfg AdsbConfig) error
	// Restart restarts a named component (only "dump1090" exists today).
...
// StatsProvider returns the live decoder figures for the heartbeat. All
// pointers, so "not reported yet" (decoder still starting) stays
// distinguishable from a real zero.
type StatsProvider func() AdsbStats

// AdsbStats are the decoder figures relayed to the backend each heartbeat.
type AdsbStats struct {
	AircraftNow *int
	MsgRate     *float64
	MaxRangeKm  *float64
	GainNow     *float64
...
type depthProvider interface {
	// Expired counts snapshots dropped for exceeding the queue's age bound —
	// the link was down long enough that the positions became worthless.
	Expired() uint64
	Depth() int
}
...
// config + services restart cmds; status supplies the component states; stats
// supplies the live decoder figures for the heartbeat (may be nil).
func New(
	cfg *agentcfg.Config,
	q depthProvider,
	applier ConfigApplier,
	...
	c := &Client{cfg: cfg, q: q, applier: applier, status: status, stats: stats}
...
func (c *Client) sendHello(conn *websocket.Conn) error {
	...
	h := protocol.Hello{
		...
		// This agent runs no managed SDR-Trunk/rdio components; these serialize
		// empty and are ignored by the backend for an ADS-B node.
		SDRTrunkVersion:      "",
		RdioVersion:          "",
		OS:                   runtime.GOOS,
		Arch:                 runtime.GOARCH,
...
// sendStatus builds and sends the heartbeat: component states + queue depth +
// applied config version + the live decoder figures. The radio-specific arrays
// (tuners/channels/activeCalls/events) serialize empty for an ADS-B node.
func (c *Client) sendStatus(conn *websocket.Conn) error {
	...
	if c.stats != nil {
		s := c.stats()
		st.AdsbAircraftNow = s.AircraftNow
		st.AdsbMsgRate = s.MsgRate
		st.AdsbMaxRangeKm = s.MaxRangeKm
		st.AdsbGainNow = s.GainNow
	}
...
// handleConfigPush applies a pushed ADS-B config. The apply runs on a background
...
func (c *Client) handleConfigPush(env *protocol.Envelope) {
	...
	// change, so this node keeps receiving pushes whose payload is byte-for-byte
	// what it already runs (e.g. someone edited a RADIO channel). Re-applying
	// would needlessly restart the decoder (~2s of lost reception plus a USB
	// settle), so if the pushed version matches what we've already applied, just
	// re-ack and skip the apply.
	...
	go func() {
		...
		log.Printf("wsclient: applying config version %s (capture=%t feed=%t gain=%q)",
			cfg.ConfigVersion, cfg.CaptureEnabled, cfg.FeedEnabled, cfg.Gain)
...
func LoadPersistedConfig(cfg *agentcfg.Config) (AdsbConfig, bool) {
