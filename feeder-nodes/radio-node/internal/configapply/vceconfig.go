...
// rdio API keys.
//
// The agent does NOT manage tuner assignment: sdrtrunk owns tuner allocation and
// retunes dongles to lock frequencies as it needs to. A channel is pinned to a
// specific SDR ONLY when the operator sets its per-channel SDR field (used e.g.
// for AM traffic that must always start on the same dongle); every other channel
// stays Auto (empty preferredTuner) and sdrtrunk allocates it dynamically.
func buildVceConfig(payload ConfigPayload, localKeys map[int]string, presetList string) vceConfigState {
	...
	// autoStart=false so the import brings up nothing. A channel's preferredTuner
	// comes straight from its operator-set SDR field (below) — the agent never
	// auto-assigns tuners; sdrtrunk manages allocation for every Auto channel.
